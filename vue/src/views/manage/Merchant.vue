<template>
  ...
  <div>
    ...
    <!--模态框-->
    <el-dialog title="商家信息" :visible.sync="dialogVisible" width="40%">
...
export default {
  ...
  methods:{
    ...
    save() {
      if (!this.form.id) {
        //新增 为空
        request.post("merchant/save", this.form).then(res => {
          if (res.code === '0') {
        ...
        request.post("merchant/update", this.form).then(res => {
          if (res.code === '0') {
    ...
    del(id){
      this.$confirm('您确定删除吗？', '确认删除', {type: "warning"}).then(response => {
        request.delete('/merchant/delete/' + id).then(res => {
